...
import org.cch.mall.core.express.ExpressService;
import org.cch.mall.core.express.dao.ExpressInfo;
import org.cch.mall.core.notify.NotifyService;
import org.cch.mall.core.notify.NotifyType;
import org.cch.mall.core.qcode.QCodeService;
import org.cch.mall.core.system.SystemConfig;
import org.cch.mall.core.task.TaskService;
import org.cch.mall.core.util.DateTimeUtil;
...
import org.cch.mall.db.util.OrderHandleOption;
import org.cch.mall.db.util.OrderUtil;
import org.cch.mall.core.util.IpUtil;
import org.cch.mall.wx.task.OrderUnpaidTask;
...
import static org.cch.mall.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        MallOrder order = orderService.findById(userId, orderId);
        if (null == order) {
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            MallGrouponRules rules = grouponRulesService.findById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                ...
                // （2）不允许参加自己开团的团购
                MallGroupon groupon = grouponService.queryById(userId, grouponLinkId);
        ...
        if (couponId != 0 && couponId != -1) {
            MallCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (groupon != null) {
            MallGrouponRules grouponRules = grouponRulesService.findById(groupon.getRulesId());
            ...
            List<MallGroupon> grouponList = grouponService.queryJoinRecord(groupon.getGrouponId());
            if (groupon.getGrouponId() != 0 && (grouponList.size() >= grouponRules.getDiscountMember() - 1)) {
                for (MallGroupon grouponActivity : grouponList) {
                    grouponActivity.setStatus(GrouponConstant.STATUS_SUCCEED);
                    grouponService.updateById(grouponActivity);
                }

                MallGroupon grouponSource = grouponService.queryById(groupon.getGrouponId());
