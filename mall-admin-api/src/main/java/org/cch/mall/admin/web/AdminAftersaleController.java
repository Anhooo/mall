...
import org.cch.mall.admin.service.LogHelper;
...
import static org.cch.mall.admin.util.AdminResponseCode.ORDER_REFUND_FAILED;
...
public class AdminAftersaleController {
    ...
    @Autowired
    private MallGoodsProductService goodsProductService;
    @Autowired
    ...
    public Object list(Integer orderId, String aftersaleSn, Short status,
        ...
        List<MallAftersale> aftersaleList = aftersaleService.querySelective(orderId, aftersaleSn, status, page, limit, sort, order);
    ...
    @PostMapping("/recept")
    public Object recept(@RequestBody MallAftersale aftersale) {
        Integer id = aftersale.getId();
        MallAftersale aftersaleOne = aftersaleService.findById(id);
    ...
    public Object batchRecept(@RequestBody String body) {
        ...
        for(Integer id : ids) {
            MallAftersale aftersale = aftersaleService.findById(id);
            if(aftersale == null){
                continue;
            }
    ...
    @PostMapping("/reject")
    public Object reject(@RequestBody MallAftersale aftersale) {
        Integer id = aftersale.getId();
        MallAftersale aftersaleOne = aftersaleService.findById(id);
        if(aftersaleOne == null){
